...
//
// labelLight/labelDark follow the same convention as COLOUR_BOARDS below —
// labelLight is the colour used for a label sitting ON a light square,
// labelDark for one sitting on a dark square. Each is sampled from a
// representative light-square/dark-square patch of the board's own photo
// (averaged to ride out grain/veining/creases), then pushed further
// toward the app's own near-black/near-white text colours (--bg/--text in
// main.css) for legibility, since a photo's two square tones are not
// reliably far enough apart on their own — Leather's dark square, for
// example, is a mid-brightness amber rather than a true dark tone.
...
// for the labels themselves — _coordLabelCss() below — the same approach
// lichess's own board themes use (their --cg-ccw/--cg-ccb custom
// properties): a light square's label is coloured with something drawn
...
// rather than against a single colour that has to stay legible on both.
